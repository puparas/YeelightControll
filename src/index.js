    ...
    <div class="light_${itemArray.id} light name">
        ...
        <div class="lightStatus_${itemArray.id} lightStatus">
            ...
            <div class='colorFlowForm'>
                ...
                <div class="button_cf_start">Запустить</div>
            </div>
        </div>
    </div>`
        ...
        desktopCapturer.getSources({ types: ['window', 'screen'] }).then(async sources => {
            for (const source of sources) {
                if (source.id == sourceId) {
                    ...
                    // let averageColor = getAverageColor([color1[2]], color2);
...
document.addEventListener('DOMContentLoaded', function () {
    document.querySelector('.submitTest').addEventListener('click', ()=>{
        let value = JSON.parse(document.querySelector('.testInput').value)
        sendTestCommand('0x0000000012a69210', value)
    })
